...
use crate::{Binder, Variable, VariableValue};
...
impl<'src, Term: TermReference<'src, Type = Term>> Pretty for Variable<'src, Term> {
    fn to_document(&self, parent: Option<(Operator, Side)>, layout: Layout) -> Document {
        let id = self.value.as_ptr();

        if let (LayoutVariable::Value, VariableValue::Known { value }) =
            (layout.variable, &*self.value.borrow())
        {
...
// TODO: Remove this once all term types use the same `VariableValue`
